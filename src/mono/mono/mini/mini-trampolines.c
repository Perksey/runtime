...
static MonoMethod *get_interface_implementation_idic_helper;
...
static gpointer
resolve_vtable_slot (MonoVTable *vt, int slot, gpointer **resolved_vtable_slot, MonoMethod **m, MonoError *error)
{
	gpointer res = NULL;
...
leave:
	*resolved_vtable_slot = vtable_slot;
	return res;
}

static gpointer
resolve_idic_vtable_slot (host_mgreg_t *regs, guint8 *code, int slot, MonoObject *this_arg, MonoVTable *vt, MonoError *error)
{
	gpointer res = NULL;

	g_assert (m_class_is_dynamic_interface_castable(vt->klass));
	...
	if (mono_class_is_interface (method->klass) && mono_class_interface_offset (vt->klass, method->klass) == -1) {
		MonoReflectionTypeHandle ref_type = mono_type_get_object_handle (m_class_get_byval_arg(method->klass), error);
		if (!is_ok (error))
			return NULL;

		if (!get_interface_implementation_idic_helper) {
			mono_loader_lock ();
			if (!get_interface_implementation_idic_helper) {
				get_interface_implementation_idic_helper = mono_class_get_method_from_name_checked(mono_defaults.dynamicinterfacecastablehelpers_class, "GetInterfaceImplementation", 2, 0, error);
			}
			mono_loader_unlock ();
		}

		if (!get_interface_implementation_idic_helper || !is_ok(error))
			return NULL;
		...
		MonoReflectionType *cast_ref_type = (MonoReflectionType *)mono_runtime_invoke_checked (get_interface_implementation_idic_helper, NULL, args, error);
		if (!is_ok (error))
			return NULL;
		...
		if (cast_ref_type->type->type != MONO_TYPE_CLASS)
			return NULL;

		MonoClass *impl_type = cast_ref_type->type->data.klass;
		if (impl_type != NULL) {
			MonoVTable *impl_type_vt = mono_class_vtable_checked (impl_type, error);
			if (!is_ok(error)) {
				return NULL;
			}
...
mono_vcall_trampoline (host_mgreg_t *regs, guint8 *code, int slot, guint8 *tramp)
{
	...
	if (slot < 0 && m_class_is_dynamic_interface_castable(vt->klass)) {
		res = resolve_idic_vtable_slot(regs, code, slot, this_arg, vt, error);
