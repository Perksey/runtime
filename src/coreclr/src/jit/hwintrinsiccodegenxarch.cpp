    ...
    if (genIsTableDrivenHWIntrinsic(intrinsicId, category))
    {
        ...
        switch (numArgs)
        {
            ...
            case 2:
            {
                ...
                {
                    genHWIntrinsic_R_RM(node, ins, emitActualTypeSize(Compiler::getSIMDTypeForSize(node->gtSIMDSize)), op1Reg, op2);
                }
                else
                {
                    genHWIntrinsic_R_R_RM(node, ins, emitActualTypeSize(Compiler::getSIMDTypeForSize(node->gtSIMDSize)));
                }
                break;
            }
